...
% Fill geometry matrix for cylinder (no vectorization here), but for TE
% mode we need three different Index for Ex, Ey and Hz due to leapfrog
for I = 1:nx
    for J = 1:ny
        if ((J-nx/2)^2 + (I-ny/2)^2 <= 0.15*(d0/dx)^2)
            Index(J,I) = 1;
        end
        if ((J-nx/2)^2 + (I+0.5-ny/2)^2 <= 0.15*(d0/dx)^2)
            IndexX(J,I) = 1;
        end
        if ((J+0.5-nx/2)^2 + (I-ny/2)^2 <= 0.15*(d0/dx)^2)
            IndexY(J,I) = 1;
        end
    end
end
...
for T = 1:number_of_iterations
    ...
    Ex(1:nx,1:ny-1) = K_a1(IndexX(1:nx,1:ny-1)+1).*( K_b1(IndexX(1:nx,1:ny-1)+1).*...
    ...
    Ey(1:nx-1,1:ny) = K_a1(IndexY(1:nx-1,1:ny)+1).*( K_b1(IndexY(1:nx-1,1:ny)+1).*...
